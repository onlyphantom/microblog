...
def index():
    ...
    # .has_next, .has_prev, .next_num, .prev_num are attributes of .paginate function
    next_url = url_for(
...
def login():
    ...
    # instantiate an object from the LoginForm class
    form = LoginForm()
...
def user(username):
    ...
    page = request.args.get('page', 1, type=int)
    posts = this_user.posts.order_by(Post.timestamp.desc()).paginate(
        page, app.config['POSTS_PER_PAGE'], False
    )
    next_url = url_for(
